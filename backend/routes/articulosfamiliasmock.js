// Este modulo articulosfamiliasmock.js simula la obtencion de datos de una base de datos, en este caso de la tabla ArticulosFamilias
...
// metodo get para buscar un articulo por id pasando el id por parametro
router.get('/api/articulosfamiliasmock/:id', async function (req, res) {
    let articuloFamilia = arr_ArticulosFamiliasMock.find((x) => x.IdArticuloFamilia == req.params.id);
    if (articuloFamilia) res.json(articuloFamilia);
    else res.status(404).json({ message: 'articulofamilia no encontrado' });
  });
  

// metodo post para agregar un articulo a la coleccion
...
// metodo put para actualizar un articulo por id pasando el id por parametro
